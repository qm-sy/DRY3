	...
	if( htim->Instance == htim6.Instance ) 			//timer6:T = 1ms
	{
		if( rs485_1.timrun != 0 )     //定时器运行标志位
		{
			...
			if( rs485_1.timout >=8 )
			{
				...
				rs485_1.reflag = 1;	      //超时，说明接收完毕
			}
		}

        if( rs485_2.timrun != 0 )     //定时器运行标志位
        {
            rs485_2.timout++;
            if( rs485_2.timout >=8 )
            {
                ...
                rs485_2.reflag = 1;	      //超时，说明接收完毕
            }
        }

        if( rs485_3.timrun != 0 )     //定时器运行标志位
        {
            rs485_3.timout++;
            if( rs485_3.timout >=8 )
            {
